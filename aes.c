...
/*
 ...
 - Nb = 4, pois o tamanho da chave e igual ao estado, ou seja, 16 bytes
*/
void AddRoundKey(uint8_t *estado, uint8_t *w, uint8_t r){
	...
	for (c = 0; c < Nb; c++) {
		estado[Nb*0+c] = estado[Nb*0+c]^w[4*Nb*r+4*c+0];
		estado[Nb*1+c] = estado[Nb*1+c]^w[4*Nb*r+4*c+1];
		estado[Nb*2+c] = estado[Nb*2+c]^w[4*Nb*r+4*c+2];
		estado[Nb*3+c] = estado[Nb*3+c]^w[4*Nb*r+4*c+3];
	}
}
...
*/
void Cipher(uint8_t *entrada, uint8_t *saida, uint8_t *w){
	...
	AddRoundKey(estado, w, 0);

	for (r = 1; r < round; r++) {
		SubBytes(estado);
		ShiftRows(estado);
		MixColumns(estado);
		AddRoundKey(estado, w, r);
	}

	SubBytes(estado);
	ShiftRows(estado);
	AddRoundKey(estado, w, round);
...
*/
void inv_Cipher(uint8_t *entrada, uint8_t *saida, uint8_t *w){
